...
static void ortho_form(
	...
	struct ortho_vertex perimeter[], int perimeterlen, // Shape to make
	int tile_type, // There are 21 different tile designs on the sprite sheet
	struct r* rs, int* rslen, // Pointers to add onto the caller's rendering data
	struct coll_rect* crs, int* crlen) // Pointers to add onto the caller's collision data
{
	...
	for (int i = 0; i < perimeterlen; i++) { // For each ortho_vertex...
		...
		rs[*rslen] = (struct r) {
			...
			.source_x=(this.leftTurn? 0 : tileDim)+100+tilebaseX(tile_type), .source_y=(this.leftTurn? 2*tileDim : tileDim)+tilebaseY(tile_type),
	...
	} // (Edges and corners have been drawn)
}

// #########################################################################################
// ###################### THE SAME SHIT BUT WITHOUT A SPECIFIC STRUCT ######################
// #########################################################################################

static void ortho_form_2(
	int start_x, int start_y, // Where to start making the shape
	//struct ortho_vertex perimeter[], int perimeterlen, // Shape to make ################## This is how it was before ###############################
	int per_mags[], bool per_dirs[], int perimeterlen, // Shape to make #################### This is what im gonna change throughout the function ####
	int tile_type, // There are 21 different tile designs on the sprite sheet
	struct r* rs, int* rslen, // Pointers to add onto the caller's rendering data
	struct coll_rect* crs, int* crlen) // Pointers to add onto the caller's collision data
{
	...
	for (int i = 0; i < perimeterlen; i++) { // For each ortho_vertex...
		//this = perimeter[i];
		struct ortho_vertex this = (struct ortho_vertex) { .len=per_mags[i], .leftTurn=per_dirs[i] }; // ################################################################################
		vxs[i] = x;
		vys[i] = y;
	...
	if ( (x != start_x) || (y != start_y) || vertical || negative) {
		printf("ERROR in ortho_form(): UNCLOSED SHAPE (Somehow reached line 273 btw)\n");
		exit(1);
	}
	...
	for (int i = 0; i < perimeterlen; i++) { // For each ortho_vertex...
		...
		struct ortho_vertex this = (struct ortho_vertex) { .len=per_mags[i], .leftTurn=per_dirs[i] }; // ################################################################################
		if (this.len < 2*tileDim) {
			printf("ERROR in ortho_form(): NOT ENOUGH ROOM FOR CORNER TILE (Somehow reached line 327 btw)\n");
			exit(1);
		}
		if (vertical) {
			if (this.len > 2*tileDim) {
